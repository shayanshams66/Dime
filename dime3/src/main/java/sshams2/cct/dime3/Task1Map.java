...
public class Task1Map extends Mapper<Object,Text,IntWritable,Text>{
	...
	{
		String str[] = value.toString().split(space);
		String id = str[0];
		int len = str[1].length();
		stat = 0; kmer = 0; revStat = 0; revKmer = 0;
		x = 0; y = 0;
		for(int i=0;i<len;i++){
			x = this.toHash(str[1].charAt(i));
			y = this.toHash(str[1].charAt(len - i - 1));
			y = y==4?4:3-y;
			stat = stat << 2; revStat = revStat << 2;
			kmer = kmer << 2; revKmer = revKmer << 2;
			stat += x==4?0:3; revStat += y==4?0:3;
			kmer += x; revKmer += y;
			stat &= MASK; revStat &= MASK;
			kmer &= MASK; revKmer &= MASK;
			if(stat==MASK){
				keyOut.set(kmer);
