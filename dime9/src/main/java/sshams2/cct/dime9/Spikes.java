...
public class Spikes {
	...
	public Spikes(int _nKmer, int _nLoc){
		arrLocs = new int[_nKmer][_nLoc];
		defNLoc = _nLoc;
	}
	
	public void add(int _kmer, int _loc){
	...
	private void addLoc(int _row, int _loc){
		if(this.arrLocs[_row]==null){
			this.reSizeCollum(_row);
		}
		int len = (this.arrLocs[_row][0] + 1);
		if(len>=this.arrLocs[_row].length){
			this.reSizeCollum(_row);
		}
		this.arrLocs[_row][len] = _loc;
		this.arrLocs[_row][0] = (len);
	}
	
	public void addSpikes(int _kmer, int _loc){
	...
	private void reSize(){
		int[][] arrNew = new int[this.arrLocs.length*3/2][];
	...
	private void reSizeCollum(int _idx){
		if(this.arrLocs[_idx]==null){
			this.arrLocs[_idx] = new int[defNLoc];
		}else{
			int[] arrNew = new int[this.arrLocs[_idx].length*3/2];
